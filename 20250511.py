...
df_main = pd.read_csv(r'.\エリアプライス九州.csv', parse_dates=['受渡日時刻']).rename(columns={'受渡日時刻': 'Date', 'エリアプライス九州': 'y'})

file_configs = [
    ('燃料価格まとめ.csv', {'usecols': ['Date', 'LNG', 'Coal']}),
    ('自社小売需要実績.csv', {'usecols': ['Date', 'Actual retail demand']}),
    ('気象実績まとめ_1.csv', {'usecols': ['Date',
                                   'Weighted Average_Temperature',
                                   'Weighted Average_Humidity',
                                   'Weighted Average_Solar Radiation']})
]

for path, kwargs in file_configs:
    df_tmp = pd.read_csv(path, parse_dates=['Date'], **kwargs)
    df_main = df_main.merge(df_tmp, on='Date', how='inner')

# 気象予測まとめ、需要予測まとめは1週間ごとのデータしか存在しないためにまだ使えない
# 電力、LNG先物、HJKS情報は一旦除く
    ...
    for pred_date in prediction_dates:
        ...
        for d in [1, 14]:
            ...
            if model_type == 'lgbm':
                ...
                X_train_with_const = sm.add_constant(X_train_scaled, has_constant='add')
                ...
                X_new = sm.add_constant(X_valid_scaled, has_constant='add')
                ...
                # preds, preds_lower, preds_upper = model.predict(X_new), qr_low.predict(X_new), qr_up.predict(X_new)
                preds, preds_lower, preds_upper = model.predict(X_new), model.predict(X_new), model.predict(X_new),
...
plt.title('ElasticNet 上位15特徴量')

plt.tight_layout()
plt.show()

# TODO 予測区間、特徴量グラフ、特徴量一覧


#%%

# トレンド特徴量生成　テスト中
# TODO: diffの値を変えて特徴量を作成する
...
def create_trend_features(df, d_days):
    """
    df: DataFrame with DatetimeIndex and 'value' column (30-min frequency)
    d_days: number of past days to require for valid feature computation
    Returns: DataFrame of trend features for each timestamp, starting from df.index.min()+d_days
    """
    df = df.sort_index()
    features = pd.DataFrame(index=df.index)

    # 過去の情報を使う (予測日の値は使わない)
    # First and daily differences
    features['diff_1'] = df['value'].shift(0) - df['value'].shift(1)  # 直前とその前の差
    features['diff_48'] = df['value'].shift(48) - df['value'].shift(49)  # 1日前との差
    features['diff_336'] = df['value'].shift(336) - df['value'].shift(337)  # 7日前との差

    # 1-day window rolling stats
    window = 48
    features['slope_48'] = df['value'].rolling(window).apply(
        lambda x: np.polyfit(np.arange(window), x, 1)[0], raw=True
    )
    features['roll_mean_48'] = df['value'].rolling(window).mean()
    features['roll_mean_diff'] = features['roll_mean_48'].diff(1)
    features['roll_median_48'] = df['value'].rolling(window).median()
    features['roll_median_diff'] = features['roll_median_48'].diff(1)

    # Long-window rolling mean & ratio
    features['roll_mean_336'] = df['value'].rolling(336).mean()
    features['value_to_long_mean'] = df['value'] / features['roll_mean_336']

    # Cumulative return over 1 day
    features['cum_return_48'] = df['value'] / df['value'].shift(window) - 1

    # EWMA trend
    features['ewma_48'] = df['value'].ewm(span=window).mean()
    features['ewma_trend'] = df['value'] - features['ewma_48']

    # HP filter
    trend_hp, cycle_hp = hpfilter(df['value'], lamb=129600)
    features['hp_trend'] = trend_hp
    features['hp_cycle'] = cycle_hp

    # STL decomposition trend
    stl = STL(df['value'], period=window)
    res = stl.fit()
    features['stl_trend'] = res.trend

    # LOESS (LOWESS) trend
    lowess_vals = sm.nonparametric.lowess(df['value'], np.arange(len(df)), frac=0.1, return_sorted=False)
    features['lowess_trend'] = lowess_vals

    # Restricting data to past `d_days` before the prediction date
    valid_start = df.index.min() + pd.Timedelta(days=d_days)
    return features.loc[features.index >= valid_start]
...
# テストデータ作成
test_df = pd.DataFrame({'value': test_seasonal + test_linear_trend + test_noise}, index=test_date_idx)

# 過去7日間を使用
test_feat_df = create_trend_features(test_df, d_days=7)
